...
def evaluate_step(model,  dataset, task: Tuple[int, int],
    ...
    if fast:
        ...
        return {'loss': loss.item(), 'auroc': auroc, 'f1_score': f1, 'accuracy': accuracy}
    ...
    return {'loss': loss.item(), 'mrr': mrr, 'rck1': rck1, 'rck3': rck3,
            'rck10': rck10, 'auroc': auroc, 'f1_score': f1, 'accuracy': accuracy}


def train_live_update(loggers, model,  optimizer,  scheduler, datasets,
                      **kwargs):
    ...
    for t in tqdm(task_range, desc='snapshot', leave=True):
        ...
        if t <= cfg.train.stop_live_update_after:
            ...
            for i in tqdm(range(cfg.optim.max_epoch + 1), desc='live update',
                ...
                #internal_val_perf = evaluate_step(model, prompt, datasets[1],
                ...
                if best_model_unchanged >= tol:
                    ...
                    #train_perf = train_step(model, prompt, optimizer, scheduler,
    ...
    print(f'average AUROC across all snapshots: {np.mean(auc_hist)}')
    print(f'average MRR across all snapshots (test period only): {np.mean(mrr_hist)}')
    print(f'average F1 across all snapshots (test period only): {np.mean(f1_hist)}')
    logging.info('average AUROC across all snapshots: {}'.format(np.mean(auc_hist)))
    logging.info('average MRR across all snapshots (test period only): {}'.format(np.mean(mrr_hist)))
    logging.info('average F1 across all snapshots (test period only):{}'.format(np.mean(f1_hist)))
    logging.info('Task done, results saved in {}'.format(cfg.out_dir))

    return np.mean(auc_hist), np.mean(mrr_hist), np.mean(f1_hist)
