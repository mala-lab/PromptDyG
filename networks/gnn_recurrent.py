...
from networks.layers import (GeneralLayer, GeneralMultiLayer,
...
from networks.feature_augment import Preprocess
from networks.init import init_weights
from networks.feature_encoder import node_encoder_dict, \
    edge_encoder_dict
from networks.head import head_dict
import register as register
#from head import GNNEdgeHead, GNNNodeHead
########### Layer ############
...
def GNNLayer(dim_in, dim_out, has_act=True, id=0):
    if cfg.gnn.layer_type in recurrent_layer_types: #layer_type: residual_edge_conv
...
class gnn_recurrent(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, **kwargs):
        ...
        """
        super(gnn_recurrent, self).__init__()

        
        GNNStage = stage_dict[cfg.gnn.stage_type]#stack
        GNNHead = head_dict[cfg.dataset.task]#link_pred
    ...
    def get_embed(self, batch):
        ...
        for module in self.children():
            ...
            if module is self.post_mp:
                continue
            #print(module)
            batch = module(batch)
        #ipdb.set_trace()
        return batch.node_feature
